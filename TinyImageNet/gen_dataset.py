...
parser.add_argument('--data', default='/data/crq/data', type=str, help='path to tinyimagenet dataset')
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, transform, img_path, label_path, confidence_path=None):
        ...
        assert images.shape[1:] == (64, 64, 3) or images.shape[1:] == (2, 64, 64, 3)
        if images.shape[1:] == (2, 64, 64, 3):
...
def split_dataset(batch_size, output_dir='./datasets/split/',  num_class=200, shape=64):
    '''split merged data into 5 balanced subsets'''
    transform = transforms.Compose([
            transforms.ToTensor(),
        ])
    train_set = torchvision.datasets.ImageFolder(
        root='/data/crq/data/tiny-imagenet-200/train',
        transform=transform
    )
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True,
                                        num_workers=4)
    sub_imgs = np.zeros((5, int(len(train_loader.dataset) / 5), shape, shape, 3), dtype=np.uint8)
    sub_labels = np.zeros((5, int(len(train_loader.dataset) / 5), num_class), dtype=np.float32)
    
    '''split train set according to labels'''
    images_per_class = np.zeros((num_class, int(len(train_loader.dataset) / num_class), shape, shape, 3), dtype=np.uint8)
    for img_class in range(num_class):
        idx = 0
        for x, y in train_loader:
            x = tensor2np(x).astype(np.uint8)
            for pointer in range(len(y)):
                if y[pointer] == img_class:
                    images_per_class[img_class][idx] = x[pointer]
                    idx += 1

    '''distribute each class of images fairly into subsets'''
    for img_class in range(num_class):
        for sub_id in range(5):
            sub_imgs[sub_id][img_class * int(len(train_loader.dataset) / (num_class * 5)): (img_class + 1) * int(len(train_loader.dataset) / (num_class * 5))] = images_per_class[img_class][sub_id * int(len(train_loader.dataset) / (num_class * 5)): (sub_id + 1) * int(len(train_loader.dataset) / (num_class * 5))]
            one_hot_label = np.zeros(num_class)
            one_hot_label[img_class] = 1
            sub_labels[sub_id][img_class * int(len(train_loader.dataset) / (num_class * 5)): (img_class + 1) * int(len(train_loader.dataset) / (num_class * 5))] = one_hot_label

    '''save sub datasets'''
    for idx in range(5):
        img_dest = os.path.join(output_dir, "images{}.npy".format(idx))
        label_dest = os.path.join(output_dir, "labels{}.npy".format(idx))
        if os.path.exists(output_dir) == False:
            os.makedirs(output_dir)
        np.save(img_dest, sub_imgs[idx])
        print("images saved at: ", img_dest)
        np.save(label_dest, sub_labels[idx])
        print("labels saved at: ", label_dest)
...
def main():
    ...
    for dataset_id, (k, v) in enumerate(dataset_args.items()):
            ...
            for model_arg in source_args:
                ...
                model = resnet50()
                model.load_state_dict(torch.load(model_arg['ckpt_path']))
                normalize = ops_attack.NormalizeByChannelMeanStd(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])
            ...
            checkpoint = '/data/crq/DRL/TinyImagenet/attacks_transfer/gapmodel/netG_model_epoch_9_'
            ...
            for img_id, (x, y) in enumerate(loader):
                ...
                if attacker is fia:
                    ...
                    _, adv, is_adv = attacker(source_model, x, y, source_layers[2][1][1], learning_rate=8/2550, epsilon=8/255, niters=10, dataset='TinyImagenet')
                    ...
                    ifgsm_guide = ifgsm(source_model, x, y, learning_rate=8/2550, epsilon=8/255, niters=10, dataset='TinyImagenet')
                    source_layers=list(enumerate(map(lambda name: (name, source_model._modules.get(name)), ['conv1', 'layer1', 'layer2','layer3','layer4', 'fc'])))
                    _, adv, is_adv = attacker(source_model, x, ifgsm_guide, y, source_layers[2][1][1],learning_rate=8/2550, epsilon=8/255, niters=10, dataset='TinyImagenet')
