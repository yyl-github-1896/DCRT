...
def get_imagenet_dm_conf(class_cond=False, respace="", device='cuda',
                         model_path='/data/yyl/source/DRL_crq/CIFAR-10/ckpt/dm/cifar100_uncond_50M_500K.pt'):
...
invNormalize = NormalizeInverse([0.5071, 0.4867, 0.4408],[0.2675, 0.2565, 0.2761])
Normlize_Trans = transforms.Normalize([0.5071, 0.4867, 0.4408],[0.2675, 0.2565, 0.2761])
...
def NAA(
    ...
    adversaries = NAA_attack(model, img, labels, source_layers[2][1][1], learning_rate=step_size, epsilon=epsilon, niters=num_iteration, dataset='cifar100')
...
def ci_fgsm(model, x, y, epsilon=float(16/255), dataset="cifar100", targeted=False):
    """
    reference: Wang X, He K. 
    Enhancing the transferability of adversarial attacks through variance tuning[C]//
    Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition. 2021: 1924-1933.
...
def ILA(
    ...
    Args:
        ...
        dataset: dataset the images are from, 'cifar100' | 'imagenet'
...
def Transferable_Adversarial_Perturbations(
    ...
    """Perform cifar100 TAP attack using model on images X with labels y
...
def SGM(model, x, y, epsilon=float(8/255), mi=True,dataset="cifar100", device=None):
...
def LinBP(model, x, y, epsilon=float(8/255), mi=True,dataset="cifar100", device=None):
...
def RFA(model, x, y, epsilon=float(8/255), mi=True,dataset="cifar100", device=None):
...
def IAA(model,resnet, x, y, epsilon=float(8/255), mi=True,dataset="cifar100", device=None):
...
def BIA(model, x, y,netG,epsilon=float(8/255), arch="vgg16", dataset='cifar100', device=None, sub_batch_size=20):
    """
...
def GAP2(model, x, y,netG,epsilon=float(8/255), arch="vgg16", dataset='cifar100', device=None, sub_batch_size=10):
    """
    Generate adversarial examples in smaller batches to prevent memory overflow.
    """
    x, y, model = x.cuda(), y.cuda(), model.cuda().eval()
...
def GAP(model,netG, x, y, epsilon=float(8/255), arch="vgg16", dataset='cifar100', device=None):
...
def GAPF(model, x, y, epsilon=float(8/255), arch="vgg16", dataset='cifar100', device=None):    
...
def rpa(model, x, y, epsilon=float(16/255), arch="vgg16", dataset='cifar100'):
    '''
...
def main():
    ...
    normalize = NormalizeByChannelMeanStd([0.5071, 0.4867, 0.4408],[0.2675, 0.2565, 0.2761])
    model = nn.Sequential(normalize, model)
