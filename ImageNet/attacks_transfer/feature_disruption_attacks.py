...
def run_attack(attack, use_Inc_model = False):
    ...
    for true_label in tqdm(image_dict.keys()):        
        ...
        if attack is ILA:
            ifgsm_guide = ifgsm(model, image_batch, label_batch, learning_rate=opt.learning_rate, epsilon=opt.epsilon, niters=50, dataset='imagenet')
    ...
    # output_dir = f"/data/crq/out/{attack}/res50"
    output_dir = f"/data/crq/adv1000/{attack}/res50"
  # 设置保存图像的目录
    os.makedirs(output_dir, exist_ok=True)  # 如果目录不存在，则创建

    csv_path = '/data/crq/data/val_rs.csv'  # 修改为你的 CSV 文件路径
    df = pd.read_csv(csv_path)
# 注意：确保 CSV 中 label 列的类型与 adv_images_dict 中的 true_label 类型一致（这里转换为 int）
    label_to_filename = {int(row['label']): row['filename'] for idx, row in df.iterrows()}

# 遍历 adv_images_dict，使用原始文件名保存图像
    for true_label in adv_images_dict.keys():
    # 获取对应的原始文件名，如果找不到则使用默认命名方式
        original_filename = label_to_filename.get(true_label, f"{true_label}.png")
        for idx, adv_img in enumerate(adv_images_dict[true_label]):
            if idx == 0:
            # 若只有一个对抗样本，直接使用原始文件名
                img_path = os.path.join(output_dir, original_filename)
            else:
            # 如果有多个对抗样本，则在文件名中加入后缀
                base, ext = os.path.splitext(original_filename)
                img_path = os.path.join(output_dir, f"{base}_{idx}{ext}")
...
if __name__ == '__main__':
    ...
    eps=4/255
    num_iterations=10
    step_size=eps/num_iterations
    opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_ILA_16_50_4_2_ifgsm', niters=10, epsilon=4./255., learning_rate=step_size)
    adv_image_dict = run_attack(ILA)
    opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_FIA_16_50_4_2', niters=10, epsilon=4./255., learning_rate=step_size)
    adv_image_dict = run_attack(FIA)
    # opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_TAP_16_50_4_2_Original', niters=50, epsilon=8./255., learning_rate=step_size)
    # adv_image_dict = run_attack(Transferable_Adversarial_Perturbations)
    opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_NAA_16_50_2_2', niters=10, epsilon=4./255., learning_rate=step_size)
    adv_image_dict = run_attack(NAA)    
    # opt = Namespace(modeltype='ResNet50', layerindex=2, filename='ResNet50_AA_16_50_2_2_Momentum', niters=50, epsilon=8./255., learning_rate=step_size)
