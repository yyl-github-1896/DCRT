...
parser.add_argument('--train_dir', default='/data/crq/data/train', help='the path for imagenet training data') 
...
if args.model_type == 'vgg16':
    ...
    model = Res152_all_layer.Resnet50()
    layer_idx = 5   # Conv3_8
...
if 1:
    ...
    for itr, ((images, class_label), path) in enumerate(val_loader):
        ...
        adv = torch.clamp(adv, 0.0, 1.0)
        # if not os.path.exists(f"/data/crq/adv1000/BIA/res50"):
        #     os.mkdir(f"/data/crq/adv1000/BIA/res50")
        save_dir = os.path.join('/data/crq/kmean','BIA',str(seed))
        ...
        if  1:
            ...
            # save_images(adv.detach().cpu().numpy(), img_list=path, idx=len(path), output_dir='/data/crq/adv1000/BIA/res50')
            ...
            #                      f"/data/yyl/source/DRL_crq/ImageNet/out/BIA/res50eps8/{original_label}_{current_idx}.png")
            print('Saved images.')
