...
# 训练集用imagenet的训练集训练生成器，之后生成器生成10000对应对抗样本
# Training settings
parser = argparse.ArgumentParser(description='generative adversarial perturbations')
parser.add_argument('--imagenetTrain', type=str, default='/data/crq/data/train', help='ImageNet train root')
parser.add_argument('--imagenetVal', type=str, default='/data/crq/data/val_clean', help='ImageNet val root')
...
parser.add_argument('--MaxIterTest', type=int, default=1, help='Iterations in each Epoch')
parser.add_argument('--mag_in', type=float, default=4.0, help='l_inf magnitude of perturbation')
...
parser.add_argument('--foolmodel', type=str, default='resnet50', help='model to fool: "incv3", "vgg16", or "vgg19"')
parser.add_argument('--mode', type=str, default='train', help='mode: "train" or "test"')
parser.add_argument('--perturbation_type', type=str,default='imdep',help='"universal" or "imdep" (image dependent)')
...
def test():
    ...
    for itr, ((images, class_label), path) in enumerate(val_loader):
        ...
        # if not os.path.exists(f"/data/crq/adv1000/GAP/res50"):
        #     os.mkdir(f"/data/crq/adv1000/GAP/res50")
        save_dir = os.path.join('/data/crq/kmean','GAP',str(seed))
        ...
        if  1:
            ...
            save_images(recons.detach().cpu().numpy(), img_list=path, idx=len(path), output_dir=save_dir)
            # save_images(recons.detach().cpu().numpy(), img_list=path, idx=len(path), output_dir='/data/crq/adv1000/GAP/res50')
            # for idx in range(recons.size(0)):
            #     # 提取样本的原始标签
            ...
            #     # print(label_count)
            #     torchvision.utils.save_image(recons[idx].detach(), 
            #                      f"/data/crq/out/GAP/res50eps8/{original_label}_{current_idx}.png")
            print('Saved images.')
...
def print_history():
    ...
    # Assuming test_acc_history is a torch tensor
    test_acc_history = test_acc_history.cpu().numpy()
