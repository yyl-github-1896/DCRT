...
def split_dataset(batch_size, output_dir='./datasets/split/', source='merged'):
    ...
    if source == 'merged':
        ...
        merged_images = np.load('/data/crq/DRL/CIFAR10/DCRT/merge/images.npy')  
        merged_labels = np.load('/data/crq/DRL/CIFAR10/DCRT/merge/labels.npy')  
...
def main():
    ...
    for dataset_id, (k, v) in enumerate(dataset_args.items()):
            ...
            checkpoint = '/data/crq/DRL/CIFAR10/attacks_transfer/gapmodel/netG_model_epoch_9_'
            ...
            for img_id, (x, y) in enumerate(loader):
                ...
                if attacker is fia:
                    ...
                    _, adv, is_adv = attacker(source_model, x, y, source_layers[2][1][1], learning_rate=8/2550, epsilon=8/255, niters=10, dataset='cifar10')
                    ...
                    ifgsm_guide = ifgsm(source_model, x, y, learning_rate=8/2550, epsilon=8/255, niters=10, dataset='cifar10')
                    source_layers=list(enumerate(map(lambda name: (name, source_model._modules.get(name)), ['conv1', 'layer1', 'layer2','layer3','layer4', 'fc'])))
                    _, adv, is_adv = attacker(source_model, x, ifgsm_guide, y, source_layers[2][1][1],learning_rate=8/2550, epsilon=8/255, niters=10, dataset='cifar10')
